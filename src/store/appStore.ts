...
export const useAppStore = create<AppStore>((set, get) => ({
  ...
  currentProjectName: "Untitled diagram",
  saveStatus: "idle",
  ...
  saveCurrentProject: async (options) => {
    const state = get();
    if (!state.storageAvailable || !state.spec) return;

    const now = new Date().toISOString();
    const id = state.currentProjectId ?? createProjectId();
    const project: SavedProject = {
      id,
      name: state.currentProjectName || state.spec.title,
  ...
  openProject: async (id) => {
    try {
      ...
      set({
        ...
        spec: project.diagramSpec,
        elements: project.excalidrawElements as ExcalidrawElement[],
        sceneAppState: project.appState as Partial<ExcalidrawAppState> | null,
        themeId: theme.id,
      ...
      pushScene(project.excalidrawElements as ExcalidrawElement[], theme.canvas.background);
    } catch (error) {
